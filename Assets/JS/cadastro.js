class Cliente {
  constructor(nome, cpf, email) {
    this._nome = nome;
    this._cpf = cpf;
    this._senha = Math.round(Math.random() * 600000);
    this._email = email;
...
const Tarefa = (evento) => {
  ...
  if (
    ...
    alert(
      "Obrigatório preencher todos os campos para a criação de um novo usuário"
    );
  } else {
    const x = new Cliente(nomeDigitado, cpfDigitado, emailDigitado);
